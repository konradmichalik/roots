...
import { getMocoClient, getJiraClient, getOutlookClient, getConnectedJiraIds, isJiraConnected } from './connections.svelte';
...
async function fetchAllJiraEntries(from: string, to: string): Promise<void> {
  ...
  try {
    ...
    logger.store('timeEntries', `Loaded ${allWorklogs.length} Jira worklogs from ${connectionIds.length} connection(s)`);
  } catch (error) {
...
// ---------------------------------------------------------------------------
export async function createJiraWorklog(data: JiraCreateWorklog, connectionId?: string): Promise<boolean> {
