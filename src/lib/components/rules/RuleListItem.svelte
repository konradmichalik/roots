...
<div class="group/rule relative" role="listitem">
  <ContextMenu.Root>
    <ContextMenu.Trigger>
      <div
        class="rounded-lg border bg-card transition-all duration-150
          {isStale
          ? 'border-warning/50'
          : !rule.enabled
    ...
    <ContextMenu.Content>
      ...
      <ContextMenu.Item onclick={toggleEnabled}>
        {#if rule.enabled}
      ...
      <ContextMenu.Item onclick={onShowLog}>
  ...
  <!-- Edit overlay (hover) - rendered outside ContextMenu.Trigger so the click reaches the button -->
  <div
  ...
  >
    <button
      type="button"
      onclick={onEdit}
      class="rounded p-1 text-muted-foreground hover:text-foreground hover:bg-accent transition-colors duration-150 focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-none"
      title="Edit rule"
