<script lang="ts">
  ...
  let collapsed = $derived(settingsState.frequentlyUsedCollapsed);

  function toggleCollapsed(): void {
    updateSettings({ frequentlyUsedCollapsed: !collapsed });
  }
...
{#if pairs.length > 0}
  <div class="mb-1">
    ...
    >
      ...
      <span
        class="inline-flex items-center justify-center min-w-4 h-4 px-1 text-[10px] font-medium leading-none rounded-full bg-muted text-muted-foreground"
      >
        {pairs.length}
      </span>
    </button>
        ...
        {#each pairs as pair (pair.projectId + '-' + pair.taskId)}
          <Tooltip.Root delayDuration={400} disableHoverableContent ignoreNonKeyboardFocus>
            <Tooltip.Trigger>
              ...
              >
                <span class="truncate max-w-[220px]">{pair.customerName} — {pair.taskName}</span>
                <span
                  role="button"
                  tabindex="-1"
