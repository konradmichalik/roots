<script lang="ts">
  ...
  import PersonioConnectionForm from './PersonioConnectionForm.svelte';
  import {
    connectionsState,
    disconnectMoco,
    disconnectJira,
...
{#snippet mocoLogo()}
  <svg class="size-5" viewBox="0 0 76 76" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
{#snippet outlookLogo()}
  <svg class="size-5" viewBox="0 0 50 50" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path
...
{#snippet personioLogo()}
  <svg class="size-5" viewBox="0 0 210 212" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M156.79 41.967L138.835 9.8854C135.42 3.78512 128.994 0 122.017 0H49.6075C22.215 0.0367487 0 22.2697 0 49.6842V138.763H55.1521V70.4472C55.1521 62.0685 61.9451 55.27 70.317 55.27H140.744C148.051 55.27 153.963 61.1866 153.963 68.4996V146.811C153.963 153.169 148.822 158.35 142.433 158.35H65.3967V211.893H123.009C129.949 211.893 136.338 208.181 139.753 202.155L156.387 172.866C159.912 166.655 166.484 162.833 173.608 162.833H194.207C202.396 162.833 209.078 156.182 209.078 147.95V63.0608C209.078 55.3803 202.836 49.133 195.162 49.133H168.871C163.841 49.133 159.177 46.4136 156.717 42.0038L156.79 41.967Z"
    />
  </svg>
{/snippet}
  ...
  <Dialog.Content class="sm:max-w-lg max-h-[85vh] overflow-y-auto">
    ...
    <div class="space-y-6 py-4">
      ...
      <div class="rounded-xl border border-border p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-3">
            <div class="flex items-center justify-center text-success-text">
              {@render mocoLogo()}
            </div>
      ...
      <div class="rounded-xl border border-border p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-3">
            <div class="flex items-center justify-center text-brand-text">
              {@render jiraLogo()}
            </div>
            <h3 class="font-semibold text-foreground">Jira</h3>
          </div>
          {#if !showJiraForm && connectionsState.jiraConnections.length > 0}
            <button
              onclick={() => {
                editingJiraId = null;
                showJiraForm = true;
        ...
        {#each connectionsState.jiraConnections as conn (conn.id)}
          <div class="flex items-center justify-between py-2 border-b border-border last:border-0">
            ...
            <div class="flex items-center gap-2">
              {#if conn.state.error && !conn.state.isConnecting}
                <button
                  onclick={() => reconnectJira(conn.id)}
                  class="text-xs text-brand-text hover:text-brand-text/80 transition-colors"
                >
                  Retry
                </button>
      ...
      <div class="rounded-xl border border-border p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-3">
            <div class="flex items-center justify-center text-source-outlook-text">
              {@render outlookLogo()}
            </div>
      ...
      <div class="rounded-xl border border-border p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-3">
            <div class="flex items-center justify-center text-source-personio-text">
              {@render personioLogo()}
            </div>
            <h3 class="font-semibold text-foreground">Personio</h3>
          </div>
          {#if connectionsState.personio.isConnected}
            <div class="flex items-center gap-2">
              <span class="text-xs text-success-text">Connected</span>
              <button
                onclick={() => disconnectPersonio()}
                class="text-xs text-muted-foreground hover:text-danger-text transition-colors"
              >
                Disconnect
              </button>
